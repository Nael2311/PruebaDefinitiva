...
void ssd1306_display_text_x2(SSD1306_t * dev, int page, char * text, int text_len, bool invert)
{
    if (page >= dev->_pages) return;
    int _text_len = text_len;
    if (_text_len > 8) _text_len = 8;  // Permite hasta 8 caracteres ya que son más pequeños que en x3
    ...
    for (int nn = 0; nn < _text_len; nn++) {
        ...
        // Escalar el carácter a 2x de altura
        out_column_t out_columns[8];
        memset(out_columns, 0, sizeof(out_columns));

        for (int xx = 0; xx < 8; xx++) { // Por cada columna del carácter original

            uint32_t in_bitmask = 0b1;
            uint32_t out_bitmask = 0b11;  // Escalar a 2 bits de altura

            for (int yy = 0; yy < 8; yy++) { // Por cada píxel de la columna original
                if (in_columns[xx] & in_bitmask) {
                    out_columns[xx].u32 |= out_bitmask;  // Escalar a 2x de altura
                }
                in_bitmask <<= 1;
                out_bitmask <<= 2;  // Mover la máscara de salida 2 posiciones (escala 2x)
            }
        }

        // Renderizar el carácter en 8 columnas de alto y escalar a 2x en ancho
        for (int yy = 0; yy < 2; yy++) {  // Agrupar en 2 filas de píxeles altos

            uint8_t image[16];  // 8 columnas escaladas a 2 píxeles de ancho = 16 bytes
            for (int xx = 0; xx < 8; xx++) { // Por cada columna
                image[xx*2+0] = 
        ...
        seg = seg + 16;  // Mover el segmento en 16 bytes debido a la escala 2x
    }
}
